...
php artisan p:environment:setup --author bagou@zelkoa.fr --url https://$domaine --timezone Europe/Paris --cache file --session database --queue database --disable-settings-ui
else
php artisan p:environment:setup --author bagou@zelkoa.fr --url http://$domaine --timezone Europe/Paris --cache file --session database --queue database --disable-settings-ui
fi
...
then
		...
		echo "=============================="
		rm -rf /etc/httpd/conf.d/pterodactyl.conf

echo "<VirtualHost *:80>" >> /etc/httpd/conf.d/pterodactyl.conf
echo "  ServerName" $domaine >> /etc/httpd/conf.d/pterodactyl.conf
echo "  RewriteEngine On" >> /etc/httpd/conf.d/pterodactyl.conf
echo "  RewriteCond %{HTTPS} !=on" >> /etc/httpd/conf.d/pterodactyl.conf
echo "  RewriteRule ^/?(.*) https://%{SERVER_NAME}/$1 [R,L] " >> /etc/httpd/conf.d/pterodactyl.conf
echo "</VirtualHost>" >> /etc/httpd/conf.d/pterodactyl.conf
echo "<VirtualHost *:443>" >> /etc/httpd/conf.d/pterodactyl.conf
echo "  ServerName" $domaine >> /etc/httpd/conf.d/pterodactyl.conf
...
echo "  SSLEngine on" >> /etc/httpd/conf.d/pterodactyl.conf
echo "  SSLCertificateFile /etc/letsencrypt/live/"$domaine"/fullchain.pem" >> /etc/httpd/conf.d/pterodactyl.conf
echo "  SSLCertificateKeyFile /etc/letsencrypt/live/"$domaine"/privkey.pem" >> /etc/httpd/conf.d/pterodactyl.conf
echo "</VirtualHost> " >> /etc/httpd/conf.d/pterodactyl.conf
